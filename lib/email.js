...
class FullEmail {
  ...
  async sender(to, subject, html) {
    ...
    if (config.debug) {
      this.app.coreLogger.info(`[egg-full-email] sendMail, mailOptions: ${JSON.stringify(mailOptions)}, result: ${JSON.stringify(mailOptions)}`);
    }
...
module.exports = app => {
  ...
  assert(config.service && config.port && config.ssl && config.user && config.password && config.from && config.debug !== undefined,
